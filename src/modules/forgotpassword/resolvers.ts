...
export const resolvers: ResolverMap = {
  ...
  Mutation: {
    sendForgotPasswordEmail: async (_, { email }: GQL.ISendForgotPasswordEmailOnMutationArguments, { redis, url }) => {
    ...
    changePasswordWithToken: async (_, { newPassword, token }: GQL.IChangePasswordWithTokenOnMutationArguments, { redis }) => {
      ...
      const update = User.update({ id: userId }, { password: pass, forgotPasswordLock: false })
