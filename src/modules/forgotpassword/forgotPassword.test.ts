...
describe('Forgot password module', () => {
  test('forgot logout email', async done => {
    ...
    const token = (await createForgotPasswordEmail(
      'test',
      user.id,
      redis,
    )).split('/')[2];
    ...
    expect(
      (await client.changePasswordWithToken(newFaultyPassword, token)).data,
    ).toEqual({
    ...
    expect(
      (await client.changePasswordWithToken(newPassword, token)).data,
    ).toEqual({
